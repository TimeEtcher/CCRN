...
class CRFModule(nn.Module):
    def __init__(self, in_channels, distillation_rate=0.25,kernel=2):
        ...
        self.c2 = CondConv(self.remaining_channels, in_channels, 3,1,padding=1,K=kernel)
        self.c3 = CondConv(self.remaining_channels, in_channels, 3,1,padding=1,K=kernel)
        self.c4 = CondConv(self.remaining_channels, self.distilled_channels, 3,1,padding=1)
        self.act = activation('lrelu', neg_slope=0.05)
        self.c5 = conv_layer(in_channels, in_channels, 1)
        self.cca = CCALayer(self.distilled_channels * 4)
        #self.esa = ESA(in_channels, in_channels, nn.Conv2d)
